...
static void nca_sgmrpspmat_init(int iblk, dreal ww, mat2d_dcmplx *SgmRpspmat)
{
  ...
  dreal        wwg, wwl;
  dcmplx       xil, xir, chil, chir;
  dcmplx       lambdag, lambdal, pig, pil;
  mat2d_dcmplx *LamGmat = NULL, *LamLmat = NULL, *PiGmat = NULL, *PiLmat = NULL;
  ...
  PiGmat = mat2d_new_dcmplx();
  check_mem(PiGmat, "PiGmat");
  PiGmat->alloc(1, 1, PiGmat);
  
  PiLmat = mat2d_new_dcmplx();
  check_mem(PiLmat, "PiLmat");
  PiLmat->alloc(1, 1, PiLmat);
          ...
          for(jq = 0; jq < Nqus; ++jq) {
            ...
            for(jblk = 0; jblk < Nblk; ++jblk) {
              ...
              for(ipspblkj = 0; ipspblkj < npspblkj; ++ipspblkj) {
                ...
                if(isOptic && jq == Gstat && iq == Xstat) {
                  // Greater Pi part
                  chil = Chipsp[ispin]->ptr[ipspj][jpspi];
                  chir = Chipsp[ispin]->ptr[ipspj][ipspi];
                  
                  if(cabs(chil * conj(chir)) > thr) {
                    wwg = ww - Epsp[ipspj];
                    matfunc_bisec_dcmplx(wwg, PiG_aug[ispin], PiGmat);
                    pig = PiGmat->ptr[0][0];
                    SgmRpspmat->ptr[ipspblki][jpspblki] += 0.5 * chil * conj(chir) * pig;
                  }
                  // Lesser Pi part
                  chil = Chipsp[ispin]->ptr[ipspi][ipspj];
                  chir = Chipsp[ispin]->ptr[jpspi][ipspj];
                  
                  if(cabs(chil * conj(chir)) > thr) {
                    wwl = Epsp[ipspj] - ww;
                    matfunc_bisec_dcmplx(wwl, PiL_aug[ispin], PiLmat);
                    pil = PiLmat->ptr[0][0];
                    SgmRpspmat->ptr[ipspblki][jpspblki] += 0.5 * chil * conj(chir) * pil;
                  }
                }
              }
            }
  ...
  mat2d_del_dcmplx(PiLmat);
  mat2d_del_dcmplx(PiGmat);
...
static void nca_sgmrpspmat_iter(int iblk, dreal ww, mat2d_dcmplx *SgmRpspmat)
{
  ...
  dreal          wwg, wwl;
  dcmplx         xil, xir, chil, chir, dSgm;
  dcmplx         lambdag, lambdal, pig, pil;
  size_t         nlen;
  dreal          *xval = NULL;
  dcmplx         *lamxgrn = NULL, *pixgrn = NULL;
  mat2d_dcmplx   *LamGmat = NULL, *LamLmat = NULL, *PiGmat = NULL, *PiLmat = NULL,
                 *GrnRpspmat = NULL;
        ...
        for(iq = 0; iq < Nqus; ++iq) {
          for(jq = 0; jq < Nqus; ++jq) {
            ...
            for(jblk = 0; jblk < Nblk; ++jblk) {
              ...
              pixgrn  = (dcmplx *) calloc(ngrd, sizeof(dcmplx));
              ...
              for(ipspblkj = 0; ipspblkj < npspblkj; ++ipspblkj) {
                ...
                for(jpspblkj = 0; jpspblkj < npspblkj; ++jpspblkj) {
                  ...
                  if(isOptic && jq == Gstat && iq == Xstat) {
                    // Greater Pi part
                    chil = Chipsp[ispin]->ptr[jpspj][jpspi];
                    chir = Chipsp[ispin]->ptr[ipspj][ipspi];
                    ...
                    if(cabs(chil * conj(chir)) > thr) {
                      for(ig = 0; ig < ngrd; ++ig) {
                        ...
                        matfunc_bisec_dcmplx(wwg, PiG_aug[ispin], PiGmat);
                        pig = PiGmat->ptr[0][0];
                        pixgrn[ig] = I / (2.0*PI) * chil * conj(chir)
                          * pig * GrnRpspmat->ptr[ipspblkj][jpspblkj];
                      }
                      dSgm = numeric_quadrat_dcmplx(ngrd, xval, pixgrn);
                      SgmRpspmat->ptr[ipspblki][jpspblki] += dSgm;
                    }
                    // Lesser Pi part
                    chil = Chipsp[ispin]->ptr[ipspi][ipspj];
                    chir = Chipsp[ispin]->ptr[jpspi][jpspj];
                    ...
                    if(cabs(chil * conj(chir)) > thr) {
                      for(ig = 0; ig < ngrd; ++ig) {
                        ...
                        pil = PiLmat->ptr[0][0];
                        pixgrn[ig] = I / (2.0*PI) * chil * conj(chir)
                          * pil * GrnRpspmat->ptr[ipspblkj][jpspblkj];
                      }
                      dSgm = numeric_quadrat_dcmplx(ngrd, xval, pixgrn);
                      SgmRpspmat->ptr[ipspblki][jpspblki] += dSgm;
                    }
              ...
              freeup(pixgrn); freeup(lamxgrn); freeup(xval);
            }
          }
        }
      }
...
static void nca_sgmlpspmat_init(int iblk, dreal ww, mat2d_dcmplx *SgmLpspmat)
{
  ...
  dreal        wwl, wwg;
  dcmplx       xil, xir, chil, chir;
  dcmplx       lambdal, lambdag, pil, pig;
  mat2d_dcmplx *LamLmat = NULL, *LamGmat = NULL, *PiLmat = NULL, *PiGmat = NULL;
              ...
              for(ipspblkj = 0; ipspblkj < npspblkj; ++ipspblkj) {
                ...
                if(isOptic && jq == Gstat && iq == Xstat) {
                  ...
                  if(cabs(chil * conj(chir)) > thr) {
                    ...
                    matfunc_bisec_dcmplx(wwl, PiL_aug[ispin], PiLmat);
                    pil = PiLmat->ptr[0][0];
                    SgmLpspmat->ptr[ipspblki][jpspblki] += zeta * chil * conj(chir) * pil / Npsp;
                  }
                  // Greater Pi part
                  chil = Chipsp[ispin]->ptr[ipspi][ipspj];
                  chir = Chipsp[ispin]->ptr[jpspi][ipspj];
                  ...
                  if(cabs(chil * conj(chir)) > thr) {
                    ...
                    matfunc_bisec_dcmplx(wwg, PiG_aug[ispin], PiGmat);
                    pig = PiGmat->ptr[0][0];
                    SgmLpspmat->ptr[ipspblki][jpspblki] += zeta * chil * conj(chir) * pig / Npsp;
                  }
                }
              }
            }
                  ...
                  if(isOptic && jq == Gstat && iq == Xstat) {
                    ...
                    if(cabs(chil * conj(chir)) > thr) {
                      for(ig = 0; ig < ngrd; ++ig) {
                        ...
                        pil = PiLmat->ptr[0][0];
                        pixgrn[ig] = I / (2.0*PI) * chil * conj(chir) 
                          * pil * GrnLpspmat->ptr[ipspblkj][jpspblkj];
                      }
                      dSgm = numeric_quadrat_dcmplx(ngrd, xval, pixgrn);
                      SgmLpspmat->ptr[ipspblki][jpspblki] += dSgm;
                    }
                    ...
                    if(cabs(chil * conj(chir)) > thr) {
                      for(ig = 0; ig < ngrd; ++ig) {
                        ...
                        pig = PiGmat->ptr[0][0];
                        pixgrn[ig] = I / (2.0*PI) * chil * conj(chir)
                          * pig * GrnLpspmat->ptr[ipspblkj][jpspblkj];
                      }
                      dSgm = numeric_quadrat_dcmplx(ngrd, xval, pixgrn);
                      SgmLpspmat->ptr[ipspblki][jpspblki] += dSgm;
                    }
